...
def main():
    ...
    class_names = list(set([item for i in range(5) for item in list(data[CLASS_NAME_COL.format(i + 1)].unique())]))
    ...
    for index, row in data.iterrows():
        ...
        for i in range(5):
            class_name, prob = row[[CLASS_NAME_COL.format(i + 1), CLASS_PROB_COL.format(i + 1)]]
