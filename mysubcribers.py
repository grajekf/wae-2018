...
class Printer(Subscriber):
    def notify(self, generation, population, fitness, **kwargs):
        print(
            "Generation: %d, Min fitness: %f, Avg fitness: %f, Max fitness: %f, Fitness function uses: %d" % (generation, np.min(fitness), np.average(fitness), np.max(fitness), kwargs['current_fitness_uses']))
