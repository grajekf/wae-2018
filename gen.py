...
def tournament_generator(tournament_size):
    def select_tournament(population_with_costs):
        return max([population_with_costs[i] for i in np.random.choice(len(population_with_costs), tournament_size)],
                   key=lambda p: p[1])[0]
...
def costs_pixel_amount(original, population, costs, exponent):
    return [((p.size - np.count_nonzero((p - original).round(5))) / p.size) ** exponent * c for p, c in
...
def cost_change_original(population, probs, original_image, original_classes, exponent):
    ...
    return costs_pixel_amount(original_image, population, costs, exponent), prob, costs
...
def run(args, hook=None):
    inp, out, population_size, mutation_rate, mutation_variance, max_change, classes_to_avoid, elitism, finish = args.input, args.o, args.p, args.mr, args.mv, args.c, args.ca, args.e, args.f
